...
const Register = () => {
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      const response = await authenticate("/auth/register", "POST", {}, JSON.stringify(data))
            ...
            <div className='sm:w-[65vw] flex items-center justify-center w-full h-screen relative overflow-hidden'>
              ...
              <div className="sm:w-[50%] w-[80%] mt-[10vh] p-4 bg-white border border-[#e3e6f0] rounded-lg shadow sm:p-6 md:p-8 text-left z-10">
                  <form className="space-y-6" onSubmit={handleSubmit}>
                      <h5 className="text-xl font-bold text-gray-900 text-center">Register to ZeroPus</h5>
                      <div>
                          <label for="username" className="block mb-2 text-base font-medium text-gray-900">Username<sup className='text-primary'>*</sup></label>
                          <input type="text" name="username" id="username" className="bg-gray-50 border border-[#e3e6f0] text-gray-900 text-base rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" required />
                      </div>
                      <div>
                          <label for="email" className="block mb-2 text-base font-medium text-gray-900">Email<sup className='text-primary'>*</sup></label>
                      ...
                      <div className="text-base font-medium text-gray-500">
                          Login <Link to={"/login"} className="text-primary hover:underline">Already have an acount?</Link>
                      </div>
                  </form>
              </div>
            </div>
